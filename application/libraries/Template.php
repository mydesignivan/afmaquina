...
class Template {
	...
	{	
		...
		$this->compress = $this->ci->config->item('compress');
	...
	{	
		...
		{	
			...
			if ($this->cache_layout && !$this->is_cached())
			{
                ...
                if( $this->compress ) $output = $this->_compress_html($output);
	...
	private function _render_view($view_name='', $cache_me=false)
	{
		...
		{
                    if( $this->compress ) $content = $this->_compress_html($content);
	...
	//---------------------------------------------------------------
        private function _compress_html($content){
            $search = array(
